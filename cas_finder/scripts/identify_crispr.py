...
def run_pilercr(fasta, tmpdir):
    cmd = "pilercr -in %s -out /dev/stdout" % fasta
    p = sp.Popen(cmd, shell=True, stdout=sp.PIPE, stderr=sp.PIPE)
    o, e = [_.decode("utf-8") for _ in p.communicate()]
    c = p.returncode
    return o, e, c
...
def parse_pilercr(out, contigs):
    ...
    strings = (
        out.split("DETAIL REPORT\n")[1]
        .split("SUMMARY BY SIMILARITY\n")[0]
        .split("\n\n\nArray ")[1:]
    )
...
def run_crt(crt, fasta, tmpdir, xmx):
    """Run CRT; specify mem usage; return output; exit if non-zero exit code"""
    cmd = "java -Xmx%sm -cp %s crt -maxRL 55 %s /dev/stdout" % (xmx, crt, fasta)
    p = sp.Popen(cmd, shell=True, stdout=sp.PIPE, stderr=sp.PIPE)
    o, e = [_.decode("utf-8") for _ in p.communicate()]
    c = p.returncode
    return o, e, c
...
def parse_crt(out, contigs):
    ...
    for string in out.split("\nCRISPR ")[1:]:
        ...
        for line in string.split("\n")[3:]:
            ...
            array_item.repeat_start = (
                array_item.repeat_start - contigs[contig_index].start + 1
            )
            array_item.spacer_start = array_item.repeat_start + len(
                array_item.repeat_seq
            )
        ...
        for item_index, item in enumerate(array.items):
            ...
            if (
                item.contig_id != array.contig_id
                or
                # repeat end on next contig
                item.repeat_start + len(item.repeat_seq)
                >= contigs[array.contig_index].length
                or
                # spacer ends on next contig
                item.spacer_start + len(item.spacer_seq)
                >= contigs[array.contig_index].length
            ):
...
class Array:
    ...
    def is_truncated(self, append_ns, contig, min_dist=50):
        ...
        if self.start_pos - 1 <= min_dist:
            left = "Yes"
        else:
            left = "No"

        if len(contig.seq) - append_ns - self.end_pos - 1 <= min_dist:
            right = "Yes"
        else:
            right = "No"
...
def parse_args():
    ...
    parser.add_argument(
        "-i", dest="inpath", metavar="FASTA", required=True, help="input FASTA file"
    )
    parser.add_argument(
        "-o", dest="outdir", metavar="PATH", help="output directory", required=True
    )
    parser.add_argument(
        "-c", dest="crt", metavar="PATH", help="CRT1.2-CLI.jar file path", required=True
    )
    parser.add_argument(
        "-n",
        dest="append_ns",
...
def fasta_offsets(fasta, max_bp, max_seqs):
    """
    Split multi-fasta file into chunks of sequences, each with up to max_bp cumulative
...
def write_tmp_seqs(fasta, outdir, offset, nseqs, append_ns, concat):
    ...
    with open(tmp_path, "w") as tmp_file:
        ...
        for i in range(nseqs):
            ...
            contig.id = s.id
            contig.seq = (
                ("N" * append_ns) + str(s.seq).replace("X", "N") + ("N" * append_ns)
            )
...
def run_pipeline(args, offsets, append_ns, program, basename=None):
    arrays = []
    ...
    for offset, nseqs in offsets:
        ...
        tmp_path, contigs = write_tmp_seqs(
            args["inpath"], args["outdir"], offset, nseqs, append_ns, concat=concat_seqs
        )

        # run programs
        if program == "crt":
            out, err, code = run_crt(
                args["crt"], tmp_path, args["tempdir"], xmx=args["xmx"]
            )
    ...
    if basename is None:
        basename = program
    write_crispr_files(contigs, arrays, outbase=args["outdir"] + "/" + basename)
...
def clean_up_arrays(arrays, append_ns, contigs):
    ...
    for array in arrays:
        ...
        array.contig_length = len(contigs[array.contig_id].seq)


def write_crispr_files(contigs, arrays, outbase):
    ...
    for array in arrays:
        ...
        array.spacer_length = round(
            np.mean([len(_.spacer_seq) for _ in array.items]), 2
        )
        array.size = len(array.items) - 1
        array.repeat_length = len(array.consensus_repeat)
...
if __name__ == "__main__":
    ...
    run_pipeline(args, offsets, append_ns=50, program="crt", basename="crt")
    run_pipeline(args, offsets, append_ns=50, program="pilercr", basename="pilercr")
